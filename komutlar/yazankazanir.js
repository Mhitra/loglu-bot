...
exports.run = async (client, msg, args) => {
  ...
  if (opponent.bot) return msg.reply('Botlar ile oynayamazsın!');
		...
		try {
			...
			await db.add(`yazankazanir.${winner.first().author.id}`, 1)
			return msg.channel.send(`Hızlıymışsın! Tebrikler <@${winner.first().author.id}> Kazandın!`)
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['yazı-yarışı', 'yazankazanır','kelime-yarışması'],
  permLevel: 0,
  kategori: "eğlence"
};
