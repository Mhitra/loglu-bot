...
const kelime = ["muz","elma","armut","muz","erik","çilek","kiraz","mandalina","ayva", "kivi","incir","şeftali", "frambuaz","böğürtlen","ananas","avokado","Hinistan Cevizi","ejder meyvesi"]
...
module.exports.run = async (bot, message, args) => {
        ...
        try {
            ...
            while (cevap.length !== confirmation.length && point < 7) {
                await message.channel.send(stripIndents`
                    ${displayText === null ? '**Kelime türü meyve, kelimeyi bulunuz!**!' : displayText ? '**Çok iyisin!**' :  '**Yanlış Harf!**'}
                         **Kelime:**    \`${display.join(' ')}\`
            ...
            if (cevap.length === confirmation.length || tahmin) return message.channel.send(`**Tebrikler kelimeyi buldun! **${cevap}!`); //Kelime uzunluğu aynı olan kelimelere de doğru diyecek buraya bir bakmamız lazım
